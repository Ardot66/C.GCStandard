...
$(DLL): $(SOURCE) $(HEADERS_WILDCARD)/*.h
	gcc -Wall -Wextra -pedantic -std=c23 -O3 $(COMPILE_FLAGS) -fPIC -shared $(SOURCE) $(HEADERS) -L$(DLL_BIN) $(subst $() , -l,$(DEPEND)) -o $(DLL)

$(TESTS_EXE): $(DLL) $(TESTS) $(HEADERS_WILDCARD)/*.h
	gcc -Wall -Wextra -pedantic -std=c23 -O3 $(COMPILE_FLAGS) $(TESTS) $(HEADERS) -L $(DLL_BIN) -l$(NAME) $(subst $() , -l,$(DEPEND)) -o $(TESTS_EXE)
